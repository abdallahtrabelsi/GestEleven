...
class Users
{
  ...
  public function checkExistname_pl($name_pl)
  {
    $sql = "SELECT name_pl from  tbl_users WHERE name_pl = :name_pl";
...
<strong>Error !</strong> Email already Exists, please try another Email... !</div>';
      ...
      $sql = "INSERT INTO tbl_users(name, username, email, password, mobile,name_tm, roleid) VALUES(:name, :username, :email, :password, :mobile, :name_tm, :roleid)";
      $stmt = $this->db->pdo->prepare($sql);
...
<strong>Error !</strong> Email already Exists, please try another Email... !</div>';
  ...
  <strong>Error !</strong> Email did not Found, use Register email or password please !</div>';
    ...
    <strong>Error !</strong> Sorry, Your account is Diactivated, Contact with Admin !</div>';
        ...
        Session::set('name_tm', $logResult->name_tm);
  ...
  public function insertPlayerIntoUser($userid)
  {
    $userid = "";
    $userid = $_POST['id'];
    ...
    if ("$userid" != "") {
      foreach ($brands as $name_pl)
        // $lang = explode(",", $name_pl['$name_pl']);
      ...
      // echo $userid;
      // echo $userid;
      // $sql = "INSERT INTO tbl_users (name_pl) VALUES ('$name_pl')";
      // $sql = "UPDATE tbl_users join players on tbl_users.name_pl=players.name_pl Where id=:id";
      $stmt = $this->db->pdo->prepare($sql);
      $stmt->bindValue(':id', $userid);
      $stmt->bindValue(':name_pl', $xyz);
